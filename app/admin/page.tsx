...
const inputCls =
  "w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-red-500 transition";
const labelCls = "block text-xs font-bold uppercase tracking-wide text-zinc-400 mb-1.5";
...
export default function AdminPage() {
  ...
  const login = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setMsg({ type: "err", text: "Invalid admin key" });
    }
  };
  ...
  const remove = async (id?: string) => {
    ...
    if (res.ok) {
      setMsg({ type: "ok", text: "Article deleted" });
      load();
    }
  };
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-24">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-8">
          ...
          <p className="text-sm text-zinc-400 mb-6">Enter your admin key to manage articles.</p>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-black text-white">News Admin</h1>
        <button
          onClick={() => setAuthed(false)}
          className="text-sm text-zinc-400 hover:text-white border border-zinc-800 rounded-md px-3 py-1.5 transition"
        >
          Log out
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Form */}
        <form onSubmit={save} className="space-y-4 rounded-xl border border-zinc-800 bg-zinc-900/40 p-6 h-fit">
          ...
          <div>
            <label className={labelCls}>Content (Markdown: ## headings, - lists, blank line paragraphs)</label>
            <textarea
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white font-bold py-2.5 rounded-lg transition"
            >
            ...
            {editingId && (
              <button
                ...
                className="px-4 py-2.5 border border-zinc-700 text-zinc-300 rounded-lg hover:bg-zinc-800 transition text-sm"
              >
                Cancel
              </button>
            )}
          ...
          <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
            {news.map((item) => (
              <div
                ...
                className="flex items-start justify-between gap-3 p-3 rounded-lg border border-zinc-800 hover:border-zinc-600 transition"
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold uppercase text-red-500">{item.category}</span>
                    {item.featured && (
                      <span className="text-[10px] font-bold uppercase bg-yellow-500/20 text-yellow-400 px-1.5 py-0.5 rounded">
                        ★
                      </span>
                    )}
                  </div>
                  <p className="text-sm font-semibold text-zinc-200 truncate mt-1">{item.title}</p>
                ...
                <div className="flex gap-2 shrink-0">
                  <button
                    onClick={() => edit(item)}
                    className="text-xs px-2.5 py-1.5 rounded border border-zinc-700 text-zinc-300 hover:bg-zinc-800 transition"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => remove(item.id)}
                    className="text-xs px-2.5 py-1.5 rounded border border-red-900 text-red-400 hover:bg-red-950 transition"
                  >
                    Del
                  </button>
                </div>

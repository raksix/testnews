  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="flex items-center gap-3 mt-6 pb-6 border-b border-borderc">
        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-red-500 to-red-800 flex items-center justify-center font-bold text-white">{news.author.charAt(0)}</div>
        <div>
      ...
      {news.image && (
        <div className="mt-8 rounded-xl overflow-hidden border border-borderc">
          <img src={news.image} alt={news.title} className="w-full h-auto max-h-[500px] object-cover" />
        </div>
      )}

      {news.excerpt && (
        <p className="mt-8 text-xl text-textc font-medium leading-relaxed border-l-4 border-red-600 pl-4">{news.excerpt}</p>
      )}
      ...
      {nextItem && (
        <AutoNext nextSlug={nextItem.slug} nextTitle={nextItem.title} />
      )}

      {relatedItems.length > 0 && (
        <div className="mt-8 border-t border-borderc pt-8">
          <h2 className="text-xl font-black mb-6 text-textc">More News</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {relatedItems.map((item: NewsItem) => (
              <Link key={item.id || item.slug} href={`/news/${item.slug}`} className="group block p-4 rounded-lg border border-borderc hover:border-zinc-600 bg-surface2/40 transition">
                <span className="text-xs font-bold uppercase text-red-500">{item.category}</span>
                <h3 className="mt-1.5 font-semibold text-textc group-hover:text-white leading-snug">{item.title}</h3>
              </Link>
            ))}
          </div>
        </div>
